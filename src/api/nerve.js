 ...
 */
export const getNulsBalance = async (address, chainId = 5, assetId = 1) => await http.post('/', 'getAccountBalance', [chainId, assetId, address])
  .then((response) => {
 ...
 * @returns {Promise<AxiosResponse<any>>}
 */
export const validateTx = async (txHex) => await http.post('/', 'validateTx', [txHex])
  .then((response) => {
    // console.log(response);
    if (response.hasOwnProperty('result')) {
 ...
 * @returns {Promise<AxiosResponse<any>>}
 */
export const broadcastTx = async (txHex) => await http.post('/', 'broadcastTx', [txHex])
  .then((response) => {
    if (response.hasOwnProperty('result')) {
 ...
 * @returns {Promise<AxiosResponse<any>>}
 */
export const getSymbolPriceOfUsdt = async (chainId, assetId) => await http.postComplete('/', 'getBestSymbolPrice', [chainId, assetId])
  .then((response) => {
    if (response.hasOwnProperty('result')) {
      return response.result;
    } else {
