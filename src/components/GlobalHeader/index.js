...
const GlobalHeader = (props) => {
	...
	const connectOntoWallet = async () => {
		try {
			...
			const currentIndex = netArray.findIndex((item) => Number(item.netWorkId) === Number(chainId))
		...
		Promise.allSettled([
			...
			.then((results) => {
				if (results[0].status === 'fulfilled') {
					tdnf = results[0].value.data || DEFAULT_STAT
				}
				if (results[1].status === 'fulfilled') {
					tbusd = results[1].value.data || DEFAULT_STAT
				}
				if (results[2].status === 'fulfilled') {
					list = results[2].value.data || []
				}
			})
			.catch((err) => {
				console.log(err)
				...
				<MenuList width="max-content">
					...
					<MenuGroup title="BNB">
						<MenuItem fontWeight="bolder" boxSizing="border-box">
							<a href="https://testnet.binance.org/faucet-smart" target="_blank" rel="noreferrer">
								Get BNB
							</a>
						</MenuItem>
					</MenuGroup>
					...
					<MenuGroup title="More+">
						<MenuItem fontWeight="bolder" boxSizing="border-box" onClick={getFaucetMore}>
							History
						</MenuItem>
					</MenuGroup>
				</MenuList>
	...
	return (
		<header className={styleHeader}>
			<Box className={actionContainer} display={['none', 'none', 'flex', 'flex', 'flex']}>
				...
				{address && (
					<div
						className={styleActionContainer}
						onClick={() => {
						...
						<Box>
							<Box display="flex" justifyContent="space-between" alignItems="center">
								<Stat>
									...
									<StatHelpText>
										{t('faucet.Count')}:{healthData?.tdnf?.count || 0} / Tol:
										{healthData?.tdnf?.total || 0}
									</StatHelpText>
								</Stat>
								<Stat>
									...
									<StatHelpText>
										{t('faucet.Count')}:{healthData?.tbusd?.count || 0} / Tol:
										{healthData?.tbusd?.total || 0}
									</StatHelpText>
								</Stat>
							</Box>
							<Box>
								{healthData.list.map((l, li) => (
									<Text key={li} border="1px solid" p="1rem" color="brand.100" my=".2rem">
										...
										{l.address} |<label>{t('faucet.Amount')}:</label>
										{l.amount} |<label>{t('faucet.UpdatedAt')}:</label>
										{l.updatedAt} |<label>{t('faucet.Expired_at')}:</label>
										{l.expired_at} |<label>{t('faucet.Txhash')}:</label>
										<Link
