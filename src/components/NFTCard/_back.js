...
const NFTCard = (props) => {
	...
	return (
		...
		>
			...
			<div
				onClick={(e) => {
					handleDetail && item?.status === 'ONSALE' && handleDetail()
				}}
				style={{
			...
			<div className={styleInfoContainer}>
				<div className={styleCardHeader}>
					<div
						...
						style={{
							borderBottom:
								currentStatus.value !== 'SOLD' && _.isString(currentStatus) !== true
									? '1px solid #F3F7F9'
									: 'none',
						}}
					>
						...
						{currentStatus.value === 'ONSALE' && !isProfile && (
							<span
								style={{
									color: '#000000',
									fontSize: '12px',
							...
							>
								{item.price > 0 &&
									...
									`${Web3.utils.fromWei(String(item.price), 'ether')} ${item.type || ''}`}
							</span>
						)}
					</div>
