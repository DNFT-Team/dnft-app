...
const NftSlider = (props) => {
	...
	const renderNoData = useMemo(() => <div className={styleNoDataContainer}>{noDataSvg}</div>, [])
	const clickDetail = (item) => {
		console.log(item, 'item')
		history.push(`/market/detail?contractType=${item?.contractType}&orderId=${item?.orderId}`, {
			item,
			category: item.category,
			sortTag: item.sortTag,
		})
	}
	...
	return (
		<div className={styleArtContainer}>
			...
			<Slider {...settings}>
				{list?.length > 0 ? list.map((item, index) => renderCard(item, index)) : renderNoData}
			</Slider>
		</div>
	)
}
