...
const iframeUrl = `https://fun.dnft.world/${
	globalConfig.net_env === 'mainnet' ? 'syncbtc' : 'test_syncbtc'
}/`
...
const SyncBtcScreen = (props) => {
	...
	const getMedalInfo = useCallback(async () => {
		...
		if (wallet) {
			...
			let medalIds = await Promise.all([0, 1, 2].map((item) => myContract.methods.ids(item).call()))

			let data = await Promise.all(medalIds.map((item) => myContract.methods.tokenIds(item).call()))
	...
	const handleRaffle = useCallback(async () => {
		...
		try {
			const myIgoContract = new window.web3.eth.Contract(igoAbi, igoContract[currentNetEnv])
	...
	const getIsApproved = useCallback(async () => {
		...
		if (wallet) {
			...
			try {
				const myBusdContract = new window.web3.eth.Contract(busdAbi, busdContract[currentNetEnv])
		...
		<div className={styleContainer}>
			...
			<div className={styleBottomContainer}>
				<div>
					<div className={styleInfoContainer}>
						...
						<span className="goal">
							{t('nftCard.supply')}：{medalData.Gold.total}
						</span>
						<div
							className="raised"
							style={{ visibility: !isTestnet && 'hidden', height: !isTestnet && '16px' }}
						>
							{t('igo.minted')}：{medalData.Gold.mintedTotal}
						</div>
					</div>
					<div className={styleInfoContainer}>
						<span className="title">{t('igo.silver.nft')}</span>
						<span className="goal">
							{t('nftCard.supply')}：{medalData.Silver.total}
						</span>
						<div
							className="raised"
							style={{ visibility: !isTestnet && 'hidden', height: !isTestnet && '16px' }}
						>
							{t('igo.minted')}：{medalData.Silver.mintedTotal}
						</div>
					</div>
				</div>
				<div>
					<div className={styleInfoContainer}>
						...
						<span className="goal">
							{t('nftCard.supply')}：{medalData.Bronze.total}
						</span>
						<div
							className="raised"
							style={{ visibility: !isTestnet && 'hidden', height: !isTestnet && '16px' }}
						>
							{t('igo.minted')}：{medalData.Bronze.mintedTotal}
						</div>
					</div>
					<div className={styleInfoContainer}>
						<span className="title">{t('igo.all.nft')}</span>
						<span className="goal">
							{t('nftCard.supply')}：{medalData.Total.total}
						</span>
						<div
							className="raised"
							style={{ visibility: !isTestnet && 'hidden', height: !isTestnet && '16px' }}
						>
							{t('igo.minted')}：{medalData.Total.mintedTotal}
						</div>
					</div>
				</div>
			</div>
			...
			{showSuccess && (
				<div className={styleChampionModal}>
					...
					<span>{t('igo.claim.prize.desc')}</span>
					{!isApproved && (
						<div
			...
			{showRaffle && (
				<div className={styleMedalGif}>
					...
					/>
					<span>{t('igo.click.check.nft')}</span>
				</div>
			)}
			{showStepGif && <img className={stylStepGif} src={require('images/igo/step.gif').default} />}
			<div
			...
			>
				...
				<b>{t('igo.rules')} </b>
				<p>{t('igo.rules.tip1')}</p>
				<p>{t('igo.rules.tip2')}</p>
				<p>{t('igo.rules.tip3')}</p>
				<p>{t('igo.rules.tip4')}</p>
				<p>{t('igo.rules.tip5')}</p>
				{!isTestnet && <p>{t('igo.rules.tip6')}</p>}

				<b>{t('igo.rewards')}</b>
				<p>{t('igo.rewards.tip1')}</p>
				<p>{t('igo.rewards.tip2')}</p>
				<div
...
const styleConfirmSuccessModal = css`
	...
	transform: translate(-50%, -50%);
	top: 50%;
	background: linear-gradient(180deg, rgba(0, 0, 0, 0.25) 0%, rgba(255, 255, 255, 0.25) 100%),
		#5ae9d7;
