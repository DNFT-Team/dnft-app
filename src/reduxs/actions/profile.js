		...
		return post(`/api/v1/users/address/${params.userId}`, {}, token)
			.then((res) => {
				if (res && res.status == 200) {
					dispatch({
						type: GET_MY_PROFILE_LIST.SUCCESS,
						payload: {
							data: res.data?.data,
						},
					})
...
export const getMyProfileBatch =
	(params = {}, token) =>
	(dispatch) => {
		...
		return post('/api/v1/collection/batch', params, token)
			.then((res) => {
				if (res && res.status == 200) {
					dispatch({
						type: GET_MY_PROFILE_BATCH.SUCCESS,
						payload: {
...
export const getMyProfileOwned =
	(params = {}, token) =>
	(dispatch) => {
		...
		return post('/api/v1/trans/owned', params, token)
			.then((res) => {
				if (res && res.status == 200) {
					dispatch({
						type: GET_MY_PROFILE_OWNED.SUCCESS,
						payload: {
							data: res.data?.data?.content || [],
						},
					})
...
export const getMyProfileCreated =
	(params = {}, token) =>
	(dispatch) => {
		...
		return post('/api/v1/trans/created', params, token)
			.then((res) => {
				if (res && res.status == 200) {
					dispatch({
						type: GET_MY_PROFILE_CREATED.SUCCESS,
						payload: {
							data: res.data?.data?.content || [],
						},
					})
...
export const setProfileAddress =
	(params = {}) =>
	(dispatch) => {
		dispatch({
			type: SET_PROFILE_ADDRESS.SUCCESS,
			payload: {
	...
	(dispatch) => {
		...
		return post('/api/v1/auth/nonce', params)
			.then((res) => {
				if (res && res.status == 200) {
					dispatch({
						type: GET_MY_PROFILE_TOKEN.SUCCESS,
						payload: {
	...
	(dispatch) =>
		post('/api/v1/nft/like', params, token)
			.then((res) => {
				if (res && res.status == 200) {
					dispatch({
						type: GET_MY_PROFILE_LIKE.SUCCESS,
						payload: {
							data: params,
						},
					})
	...
	(dispatch) =>
		post('/api/v1/nft/save', params, token)
			.then((res) => {
				if (res && res.status == 200) {
					dispatch({
						type: GET_MY_PROFILE_SAVE.SUCCESS,
						payload: {
							data: params,
						},
					})
